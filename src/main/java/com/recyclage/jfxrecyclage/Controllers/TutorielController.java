...
public class TutorielController implements Initializable {
    ...
    @FXML private ComboBox<User> userCombo;
        @FXML private ComboBox<User> filterUserCombo;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        loadUsers();
                loadFilterUsers(); // Nouvelle méthode pour charger les utilisateurs dans le filtre
    ...
    private void loadFilterUsers() {
        try {
            filterUserCombo.setItems(FXCollections.observableArrayList(
                userService.getAllUsers()));
            // Ajoutez un élément vide pour permettre la réinitialisation
            filterUserCombo.getItems().add(0, null);
        } catch (SQLException e) {
            showAlert("Erreur", "Impossible de charger les utilisateurs pour le filtre: " + e.getMessage());
        }
    }
    
        // Ajoutez ces méthodes de gestion des filtres
    @FXML
    private void handleFilterByUser() {
        User selectedUser = filterUserCombo.getValue();
        if (selectedUser != null) {
            try {
                List<Tutoriel> filteredTutoriels = tutorielService.getTutorielsByUser(selectedUser.getId());
                tutorielTable.setItems(FXCollections.observableArrayList(filteredTutoriels));
            } catch (SQLException e) {
                showAlert("Erreur", "Impossible de filtrer les tutoriels: " + e.getMessage());
            }
        }
    }
    ...
    private void handleResetFilter() {
        filterUserCombo.setValue(null);
        loadTutoriels(); // Recharge tous les tutoriels
    }
